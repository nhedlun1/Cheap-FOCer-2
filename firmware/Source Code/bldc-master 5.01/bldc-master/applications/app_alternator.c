/*
	Copyright 2021 Niklas Hedlund	nhedlund@gmail.com
...
// Threads
static THD_FUNCTION(alternator_thread, arg);
static THD_WORKING_AREA(alternator_thread_wa, 2048);
...
void app_alternator_start(void) {
	mc_interface_set_pwm_callback(pwm_callback);

	stop_now = false;
	chThdCreateStatic(alternator_thread_wa, sizeof(alternator_thread_wa),
			NORMALPRIO, alternator_thread, NULL);
...
void app_alternator_configure(app_configuration *conf) {
	(void)conf;
}
...
static THD_FUNCTION(alternator_thread, arg) {
	...
	chRegSetThreadName("App Alternator");
